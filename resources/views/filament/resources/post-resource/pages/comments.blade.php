        ...
        <div class="space-y-6">
          @forelse ($post->comments as $comment)
            <div class="flex items-start space-x-6">
              <img src="{{ asset('images/default-avatar.webp') }}" alt="avatar"
              ...
              <div class="flex-1 mx-1 px-1 p-3">
                <div class="flex justify-between items-center">
                  ...
                  <button wire:click="deleteComment({{ $comment->id }})"
                    ...
                    class="text-red-500 hover:text-red-700 my-3">Delete</button>
                </div>

                <p class="text-sm text-gray-600 mt-1">{!! $comment->comment !!}</p>

                <button wire:click="startReply({{ $comment->id }})"
                  class="inline-block text-blue-600 hover:text-blue-800 mt-3">Reply</button>

                @if($replyingTo === $comment->id)
                  <div class="mt-3 space-y-2">
                    <textarea wire:model="replyText" class="w-full border rounded p-2" rows="2"
                    ...
                    <div class="flex-1">
                      <div class="flex justify-between items-center">
                        ...
                        <button wire:click="deleteReply({{ $reply->id }})"
                          ...
                          class="text-red-500 hover:text-red-700 my-5">Delete</button>
                      </div>
