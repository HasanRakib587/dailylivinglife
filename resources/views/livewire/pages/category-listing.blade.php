            ...
            @foreach ($category->posts as $post)
                <div class="card border-0 text-center" wire:key='{{ $post->id }}'>
                    <a wire:navigate href="{{ route('post.single', $post->slug) }}">
                        <img src="{{ Storage::url($post->cover_image) }}" class="card-img-top" 
                        alt="{{ $post->title }}">
                    </a>
                    <div class="card-body">
                        <a class="text-decoration-none text-dark" wire:navigate href="{{ route('post.single', $post->slug) }}">
                            <h5 class="card-title">{{ $post->title }}</h5>
                        </a>
                        <p class="card-text">
                            {!! Str::words($post->content, 10) !!}
                        </p>
                        <a wire:navigate href="{{ route('post.single', $post->slug) }}" class="text-decoration-none text-dark"><span class="text-danger">{{ count($post->comments) }}</span> comments</a>
                    </div>
                </div>
            @endforeach
        </div>
        ...
        <div class="row">
            <div class="my-3 col-md-12 d-flex justify-content-around">
                @foreach ($subcategories as $child)
                    ...
                    href="{{ route('category.listing', $child->slug) }}"
                        ><h3>{{ $child->name }}</h3></a
                    >                
                @endforeach       
            </div>
                        ...
                        @foreach ($posts as $post)
                            <div class="col-md-5 ms-5" wire:key='{{ $post->id }}'>
                                <div class="card border-0">
                                    ...
                                    <a 
                                        href="{{ route('post.single', $post->slug) }}" class="text-decoration-none text-dark"
            ...
            @foreach ($subcategories as $child)
                ...
                <div class="col-md-3">
                    <div class="card border-0">
                        <a href="">
                        <img
                            src="{{ Storage::url($child->latestPost->cover_image) }}"
                            class="card-img-top rounded-0"
                            alt="{{ $child->name }}"
                        />
                        </a>
                        ...
                        <a
                            href=""
                            class="card-text text-center text-dark text-decoration-none"
                        >
                            <h4>{{ $child->name }}</h4>
                        </a>
                        </div>
